...
let _Tray = null; try { _Tray = require("./Tray"); } catch (_) {}
...
function _unicodeShift(s, loOff, hiOff, exceptions, digitOff) {
    return [...s].map(ch => {
        ...
        if (digitOff != null && cp >= 0x30 && cp <= 0x39) return String.fromCodePoint(cp + digitOff);
        return ch;
    }).join("");
}
...
function applyUnicodeStyle(s, style) {
    if (style && style.includes("+")) return style.split("+").reduce((acc, st) => applyUnicodeStyle(acc, st.trim()), s);
    ...
    if (!entry) return s;
    return _unicodeShift(s, entry[0], entry[1], entry[2], entry[3]);
}
...
function resolveUnicodeStyle(adv, now) {
    if (adv.styleAlternateEnabled) {
        ...
        const list = (adv.styleAlternateList ? adv.styleAlternateList.split(",").map(x => x.trim()).filter(Boolean) : null) || [adv.styleAlternateStyleA || "bold", adv.styleAlternateStyleB || "italic"];
        const idx = adv.styleAlternateRandom ? Math.floor((Math.abs(Math.sin(bucket * 12.9898)) % 1) * list.length) : bucket % list.length;
        return { style: list[idx], bucket };
    }
...
class StatusChangerBase {
    constructor(playbackState, savedStatus, gatewayClient) {
        ...
        this._lastMoodHeart = null;
        this._lastMoodHeartAt = 0;
    }

    _discordPatch(body, token) {
        return fetch("https://discord.com/api/v10/users/@me/settings", {
            method: "PATCH",
            headers: { "Content-Type": "application/json", "Authorization": token || Settings_1.Settings.credentials.token, "X-Super-Properties": ClientIdentity_1.ClientIdentity.superProperties(), "X-Discord-Locale": "en-US", "User-Agent": ClientIdentity_1.ClientIdentity.userAgent() },
            body: JSON.stringify(body)
        });
    }


    updateProfileColor() { return; /* disabled */ }

    changeStatusRequest(text, token, emoji, mergedLines, sentLine) {
        if (this._tokenInvalid) { return Promise.resolve(); }
        ...
        if (Settings_1.Settings.gateway?.enabled && this._gateway && this._gateway.connected) {
            ...
            if (sent === "hold") {
                Debug_1.Debug.write('[StatusChanger] GW post-READY hold — rolling back for retry');
        ...
        request.then(res => {
            ...
            if (res.status === 429) {
                ...
                if (!this._tokenInvalid) {
                    this._tokenInvalid = true;
                    const msg = "Discord token is invalid or revoked — status updates stopped. Update your token in the panel and restart.";
                    console.error("\x1b[31m[lyrics-status] " + msg + "\x1b[0m");
                    Debug_1.Debug.write("[StatusChanger] " + msg);
                    if (_Tray) { try { _Tray.notify("Lyrics Status", msg); } catch (_) {} }
                }
            } else {
                res.text().then(b => Debug_1.Debug.write(`[StatusChanger] Error HTTP ${res.status}: ${b}`)).catch(() => {});
                if (mergedLines) for (const ml of mergedLines) { if (!this._rollbackLines) this._rollbackLines = new Set(); this._rollbackLines.add(ml); }
                this._lastAnchorLine = null; this._lastSentText = ""; this._lastSentAt = 0;
            }
        }).catch(err => {
            Debug_1.Debug.write(`[StatusChanger] Fetch error: ${err}`);
            if (mergedLines) for (const ml of mergedLines) { if (!this._rollbackLines) this._rollbackLines = new Set(); this._rollbackLines.add(ml); }
            this._lastAnchorLine = null; this._lastSentText = ""; this._lastSentAt = 0;
        }).finally(() => { this._inFlightText = null; });
        return request;
    }
    ...
    buildMergedLines(lines, anchorIndex, mergeWindowMs, ignoreStale = false, maxLines = 0, staticWindowMs = 0) {
        const anchor     = lines[anchorIndex];
        const win        = staticWindowMs || mergeWindowMs;
        ...
        const lyricLines  = mergedLines.map(l => sanitizeLyric(l.text || ""));
        const joinedLines = lyricLines.map((l, i) =>
            i === 0 || endsWithTerminal(lyricLines[i - 1]) ? l : lcFirst(l)
        );
        const sep = Settings_1.Settings.rateLimit?.mergeSeparator ?? " ";
        return { forwardLines, mergedText: joinedLines.join(sep), lyricLines, joinedLines, mergedLines };
    }
    ...
    _iOSSync(text, emoji, force = false) {
        ...
        const now = Date.now();
        if (!force && now - this._iOSSyncSentAt < 10000) return; if (now < this._rateLimitedUntil) return;
...
function applyWordStyles(text,styleMap,start=0){
...
exports.moodHeart = moodHeart;
function applyCharStyles(text,styleMap,start=0){
    if(!styleMap||!styleMap.length)return text;
    const cps=[...text]; let wi=start;
    return cps.map(ch=>/\s/.test(ch)?ch:applyUnicodeStyle(ch,styleMap[wi++%styleMap.length])).join('');
}
const _MOOD_HEARTS = [
    ["\u2764\uFE0F", ["love","heart","kiss","baby","forever","mine","valentine","crush","darling","sweetheart","romance","adore","devotion","soulmate","embrace","cherish","gorgeous","honey","babe","lover","desire","passion","tender","falling for you","hold you","my girl","my boy","beautiful","angel","sweetie","boo","baby girl","true love","devoted","infatuated","smitten","yours","my heart","heartbeat","butterflies","chemistry","attraction","magnetic","irresistible","wifey","hubby","my queen","my king","ride or die","til death","wedding","marry","propose","hand in hand","by your side","you're the one","love of my life"]],
    ["\uD83D\uDC9B", ["happy","dance","party","smile","sunshine","celebrate","joy","laugh","fun","bright","alive","free","good times","vibe","shine","glow","upbeat","cheer","bliss","euphoric","jump","sing along","good vibes","light up","laughing","smiling","grin","delight","thrilled","excited","energetic","lively","sparkle","radiant","glee","merry","festive","carefree","playful","giddy","elated","jubilant","buzzing","pumped","stoked","on top of the world","best day","living my life","feeling good","no worries","let loose","turn up","good mood","high spirits"]],
    ["\uD83D\uDC99", ["cry","tears","alone","miss you","broken","goodbye","hurt","lonely","sad","heartbreak","sorrow","grief","empty","lost","regret","fading","weep","ache","drowning","falling apart","can't breathe","shattered","numb","hollow","missing you","without you","depressed","despair","misery","suffering","wounded","scarred","abandoned","rejected","heartbroken","devastated","crushed","gloomy","melancholy","downhearted","teardrops","sobbing","mourning","grieving","torn apart","left behind","unwanted","hopeless","helpless","empty inside","broken heart","nothing left","it's over"]],
    ["\uD83D\uDC9A", ["calm","easy","slow","drift","float","breathe","mellow","peace","quiet","gentle","soft","still","serene","soothing","tranquil","rest","ease","slow down","take it easy","unwind","settle","relax","peaceful","hush","soft breeze","stillness","calm down","steady","gentle breeze","quiet night","soothe","comfort","easygoing","laid back","chill","cozy","safe and warm","secure","grounded","balanced","centered","at peace","no rush","take your time","breathe in","breathe out","let go","slow motion","drifting away","float away","calm waters","quiet mind"]],
    ["\uD83D\uDDA4", ["hate","rage","fight","scream","burn","dark","fire","mad","anger","fury","revenge","betray","lies","war","destroy","bitter","resent","blood boil","smash","fed up","sick of","screaming","furious","enraged","livid","seething","wrath","vengeance","betrayal","deceit","backstab","traitor","enemy","battle","violence","destruction","chaos","boiling point","snap","explode","lash out","can't stand you","done with you","had enough","burn it down","tear it apart","no mercy","bitter end","cold blooded","ruthless","merciless"]]
];
function moodHeart(text){
    if(!text)return null;
    const t=text.toLowerCase();
    for(const [heart,words] of _MOOD_HEARTS){
        for(const w of words){ if(t.includes(w)) return heart; }
    }
    return null;
}
exports.applyCharStyles = applyCharStyles;
