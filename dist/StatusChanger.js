...
class StatusChanger {
    ...
    changeStatus() {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.time < (playbackState.songProgress + offset)) {
                if (!line.text)
                    continue;
                if (nextLine && nextLine.time < (playbackState.songProgress + offset))
                    continue;
                if (line === playbackState.currentLine)
                    break;
                playbackState.currentLine = line;
                if (Settings_1.Settings.view.advanced.enabled) {
                    this.changeStatusRequest(this.parseStatusString(Settings_1.Settings.view.advanced.customStatus), Settings_1.Settings.token, Settings_1.Settings.view.advanced.customEmoji);
                }
                else {
                    this.changeStatusRequest(this.getStatusString(line), Settings_1.Settings.token, "🎶");
                }
                break;
            }
        }
    ...
    getStatusString(line) {
        return `${Settings_1.Settings.view.timestamp ? `[${this.formatSeconds(+(line.time / 1000).toFixed(0))}] ` : ""}${Settings_1.Settings.view.label ? "Song lyrics - " : ""}${line.text.replace("♪", "🎶")}`;
    }
