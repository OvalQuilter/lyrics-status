...
function _fetchT(url, opts, ms) { const ac = new AbortController(); const t = setTimeout(() => ac.abort(), ms || 10000); return fetch(url, Object.assign({}, opts, { signal: ac.signal })).finally(() => clearTimeout(t)); }
...
class SpotifyDealerClient {
    ...
    async _getAccessToken() {
        ...
        const res = await _fetchT(TOKEN_URL, {
            headers: {
                "accept": "*/*",
    ...
    async _subscribe(token, connId) {
        ...
        const r1 = await _fetchT(`${NOTIFY_URL}?connection_id=${encodeURIComponent(connId)}`, {
            method: "PUT",
        ...
        const r2 = await _fetchT(REGISTER_DEVICE_URL, {
            method: "POST",
            headers: { "Authorization": `Bearer ${token}`, "Content-Type": "application/json" },
            body: JSON.stringify({
        ...
        const r3 = await _fetchT(`${CONNECT_STATE_URL}${deviceId}`, {
            method: "PUT",
            headers: { "Authorization": `Bearer ${token}`, "Content-Type": "application/json" },
    ...
    async connect() {
        ...
        let ws;
        try { ws = new WebSocket(`${DEALER_URL}?access_token=${encodeURIComponent(this._token)}`); }
        catch (e) { Debug_1.Debug.write(`[Dealer] WS create failed: ${e.message}`); this._connecting = false; this._scheduleReconnect(); return; }
        this._ws = ws;

        ws.on("open", () => {
            this._hbInterval = setInterval(() => { if (!this._ws) return; if (Date.now() - this._lastMsgAt > 65000) { Debug_1.Debug.write("[Dealer] No data in 65s -- forcing reconnect"); this._ws.terminate(); return; } if (this._ws.readyState === 1) { try { this._ws.send(JSON.stringify({ type: "ping" })); } catch (e) { Debug_1.Debug.write(`[Dealer] ping send failed: ${e.message}`); } } }, 30000);
        ...
        ws.on("message", async (data) => {
            ...
            // Ping — reply pong
            if (msg.type === "ping") { try { ws.send(JSON.stringify({ type: "pong" })); } catch (e) { Debug_1.Debug.write(`[Dealer] pong send failed: ${e.message}`); } return; }
    ...
    _scheduleReconnect() {
        ...
        if (this._reconnectAttempts >= 10) {
            ...
            if (typeof this.onFailed === "function") this.onFailed();
            return;
        }
        ...
        this._reconnectTimer = setTimeout(() => {
            ...
            this.connect().catch(e => Debug_1.Debug.write(`[Dealer] connect() rejected: ${e && e.stack || e}`));
