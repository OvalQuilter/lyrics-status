...
class Settings {
    static save() {
        try { writeFileSync("./settings.json", JSON.stringify(Object.fromEntries(Settings._KEYS.map(k => [k, this[k]])))); }
    ...
    static load() {
        ...
        catch (e) { console.warn("[lyrics-status] Could not read settings.json — using defaults. (" + e.message + ")"); Debug_1.Debug.write("Failed to read settings.json: " + e.stack); return; }
