...
const LyricsStatusApp_1 = require("./app/LyricsStatusApp");
const Settings_1 = require("./Settings");
const DataStore_1 = require("./app/DataStore");
...
function createWindow() {
    ...
    const iconPath = process.platform === "win32"
        ? (0, path_1.join)(__dirname, "../res/icon.ico")
        : (0, path_1.join)(__dirname, "../res/icon.png");
    ...
    mainWindow.loadFile((0, path_1.join)(__dirname, "../static/index.html"));
...
function setupIPC() {
    ...
    // Settings
    electron_1.ipcMain.handle("settings:get", () => Settings_1.Settings.getAll());
    electron_1.ipcMain.handle("settings:set", async (_event, settings) => {
        Settings_1.Settings.setAll(settings);
        Settings_1.Settings.save();
        // Auto-start if not running and settings look configured
        if (!lyricsApp?.isRunning && Settings_1.Settings.credentials.token && Settings_1.Settings.credentials.refreshToken) {
    ...
    // App controls
    electron_1.ipcMain.handle("app:start", async () => {
        await lyricsApp?.start();
        updateTrayMenu();
        return true;
    });
    electron_1.ipcMain.handle("app:stop", () => {
        lyricsApp?.stop();
        updateTrayMenu();
        return true;
    });
    electron_1.ipcMain.handle("app:toggle", () => {
        lyricsApp?.toggle();
        updateTrayMenu();
        return lyricsApp?.isRunning;
    });
    electron_1.ipcMain.handle("app:status", () => ({
    ...
    // Spotify auth
    electron_1.ipcMain.handle("spotify:authorize", () => {
        const clientId = Settings_1.Settings.credentials.clientID;
        const redirectUri = Settings_1.Settings.credentials.customRedirectUri;
        if (Settings_1.Settings.credentials.useExternalAuthServer) {
            electron_1.shell.openExternal(`https://rocky-quintessential-island.glitch.me/login/${Settings_1.Settings.credentials.uuid}`);
        }
        else {
            ...
            electron_1.shell.openExternal(`https://accounts.spotify.com/authorize?client_id=${clientId}&response_type=code&redirect_uri=${encodeURIComponent(redirectUri)}&scope=${scope}`);
        }
    });
    // Play a song from history
    electron_1.ipcMain.handle("spotify:play", async (_event, data) => {
        try {
            let trackUri = data.uri;
            // If no URI provided, search for the track
            if (!trackUri && data.name && data.artist) {
                trackUri = (await SpotifyService_1.SpotifyService.searchTrack(data.name, data.artist)) ?? undefined;
            }
            if (!trackUri) {
                return { success: false, error: "Could not find track" };
            }
            const success = await SpotifyService_1.SpotifyService.playTrack(trackUri);
            return {
                success,
    ...
    electron_1.ipcMain.handle("spotify:pause", () => {
        // Clear Discord status IMMEDIATELY - don't wait for Spotify
        if (lyricsApp) {
            lyricsApp.clearDiscordStatus();
        }
        // Fire and forget - don't await
        SpotifyService_1.SpotifyService.pause();
        return { success: true };
    });
    electron_1.ipcMain.handle("spotify:resume", () => {
        // Fire and forget - don't await
        SpotifyService_1.SpotifyService.resume();
        return { success: true };
    });
    electron_1.ipcMain.handle("spotify:next", () => {
        SpotifyService_1.SpotifyService.next();
        return { success: true };
    });
    electron_1.ipcMain.handle("spotify:previous", () => {
        SpotifyService_1.SpotifyService.previous();
        return { success: true };
    });
    electron_1.ipcMain.handle("token:validate", async (_event, token) => {
        try {
    ...
    electron_1.ipcMain.handle("discord:extract-token", async () => {
        const fs = require('fs');
        ...
        const getMasterKey = (localStatePath) => {
            try {
        ...
        const decryptToken = (encryptedBase64, masterKey) => {
            try {
    ...
    electron_1.ipcMain.handle("discord:dump-leveldb", async () => {
        const fs = require('fs');
        const path = require('path');
        const os = require('os');
        const { Level } = require('level');
        ...
        for (const { name, dbPath } of discordPaths) {
            ...
            try {
                ...
                files.forEach((f) => outputLines.push(`  - ${f}`));
                outputLines.push('');
            }
            catch (e) {
                outputLines.push(`Error listing files: ${e.message}`);
            }
            // Try to read LevelDB
            ...
            try {
                ...
                try {
                    ...
                    outputLines.push(`DB copy failed: ${e.message}`);
                }
                const dbToRead = fs.existsSync(tempDbPath) ? tempDbPath : dbPath;
                ...
                outputLines.push(`\nLevelDB read error: ${e.message}`);
                outputLines.push(`Stack: ${e.stack}`);
                ...
                try {
                    ...
                    for (const file of files) { // ALL files
                        ...
                        try {
                            ...
                            outputLines.push(`  Error reading: ${fe.message}`);
                        }
                    }
                }
                catch (fe) {
                    outputLines.push(`File scan error: ${fe.message}`);
                }
            }
            // Cleanup
            try {
    ...
    electron_1.ipcMain.handle("discord:user", async () => {
    ...
    electron_1.ipcMain.handle("spotify:user", async () => {
        return await SpotifyService_1.SpotifyService.getUserProfile();
    });
    // Spotify playlists
    electron_1.ipcMain.handle("spotify:playlists", async () => {
        return await SpotifyService_1.SpotifyService.getPlaylists();
    });
    // Spotify playlist tracks
    electron_1.ipcMain.handle("spotify:playlist-tracks", async (_event, playlistId) => {
        return await SpotifyService_1.SpotifyService.getPlaylistTracks(playlistId);
    });
    // Spotify liked songs
    electron_1.ipcMain.handle("spotify:liked-songs", async () => {
        return await SpotifyService_1.SpotifyService.getLikedSongs();
    });
    // Spotify seek
    electron_1.ipcMain.handle("spotify:seek", async (_event, positionMs) => {
        return await SpotifyService_1.SpotifyService.seek(positionMs);
    });
    // Spotify volume
    electron_1.ipcMain.handle("spotify:volume", async (_event, volumePercent) => {
        return await SpotifyService_1.SpotifyService.setVolume(volumePercent);
    });
    // Spotify devices
    electron_1.ipcMain.handle("spotify:devices", async () => {
        return await SpotifyService_1.SpotifyService.getDevices();
    });
    // Spotify transfer playback
    electron_1.ipcMain.handle("spotify:transfer", async (_event, deviceId, play) => {
        return await SpotifyService_1.SpotifyService.transferPlayback(deviceId, play ?? true);
    });
    // Spotify recently played
    electron_1.ipcMain.handle("spotify:recently-played", async (_event, limit) => {
        return await SpotifyService_1.SpotifyService.getRecentlyPlayed(limit ?? 20);
    });
    // Spotify audio features
    electron_1.ipcMain.handle("spotify:audio-features", async (_event, trackId) => {
        return await SpotifyService_1.SpotifyService.getAudioFeatures(trackId);
    });
    // Spotify save track (heart)
    electron_1.ipcMain.handle("spotify:save-track", async (_event, trackId) => {
        return await SpotifyService_1.SpotifyService.saveTrack(trackId);
    });
    // Spotify remove track (unheart)
    electron_1.ipcMain.handle("spotify:remove-track", async (_event, trackId) => {
        return await SpotifyService_1.SpotifyService.removeTrack(trackId);
    });
    // Spotify check if track is saved
    electron_1.ipcMain.handle("spotify:is-track-saved", async (_event, trackId) => {
        return await SpotifyService_1.SpotifyService.isTrackSaved(trackId);
    });
    // Spotify search tracks
    electron_1.ipcMain.handle("spotify:search-tracks", async (_event, query, limit) => {
        return await SpotifyService_1.SpotifyService.searchTracks(query, limit ?? 20);
    });
    // Spotify search artists
    electron_1.ipcMain.handle("spotify:search-artists", async (_event, query, limit) => {
        return await SpotifyService_1.SpotifyService.searchArtists(query, limit ?? 20);
    });
    // Spotify get artist details
    electron_1.ipcMain.handle("spotify:artist", async (_event, artistId) => {
        return await SpotifyService_1.SpotifyService.getArtist(artistId);
    });
    // Spotify get artist top tracks
    electron_1.ipcMain.handle("spotify:artist-top-tracks", async (_event, artistId) => {
        return await SpotifyService_1.SpotifyService.getArtistTopTracks(artistId);
    });
    // Spotify get album details
    electron_1.ipcMain.handle("spotify:album", async (_event, albumId) => {
        return await SpotifyService_1.SpotifyService.getAlbum(albumId);
    });
    // Spotify get album tracks
    electron_1.ipcMain.handle("spotify:album-tracks", async (_event, albumId) => {
        return await SpotifyService_1.SpotifyService.getAlbumTracks(albumId);
    });
    // Spotify get track details
    electron_1.ipcMain.handle("spotify:track", async (_event, trackId) => {
        return await SpotifyService_1.SpotifyService.getTrack(trackId);
    });
    // Spotify get recommendations
    electron_1.ipcMain.handle("spotify:recommendations", async (_event, seedTracks, seedArtists, limit) => {
        return await SpotifyService_1.SpotifyService.getRecommendations(seedTracks, seedArtists, limit ?? 20);
    });
    // Spotify get available genres
    electron_1.ipcMain.handle("spotify:genres", async () => {
        return await SpotifyService_1.SpotifyService.getAvailableGenres();
    });
    // Spotify get new releases
    electron_1.ipcMain.handle("spotify:new-releases", async (_event, limit) => {
        return await SpotifyService_1.SpotifyService.getNewReleases(limit ?? 20);
    });
    // Spotify get featured playlists
    electron_1.ipcMain.handle("spotify:featured-playlists", async (_event, limit) => {
        return await SpotifyService_1.SpotifyService.getFeaturedPlaylists(limit ?? 20);
    });
    // Song history
    electron_1.ipcMain.handle("history:get", () => lyricsApp?.getHistory() ?? []);
    electron_1.ipcMain.handle("history:clear", () => {
        lyricsApp?.clearHistory();
        return true;
    });
    // Open external links
    electron_1.ipcMain.on("open:external", (_event, url) => electron_1.shell.openExternal(url));
    // Forward app events to renderer
    exports.appEvents.on("song:changed", (data) => {
        mainWindow?.webContents.send("song:changed", data);
    });
    exports.appEvents.on("lyrics:updated", (data) => {
        mainWindow?.webContents.send("lyrics:updated", data);
    });
    exports.appEvents.on("status:sent", (data) => {
...
electron_1.app.whenReady().then(async () => {
    ...
    (0, Settings_1.initSettingsPath)(userDataPath);
    (0, DataStore_1.initDataPath)(userDataPath);
    Settings_1.Settings.load();
    DataStore_1.dataStore.reload(); // Reload data from proper path
    // Set up callback for when Spotify auth completes
    (0, Server_1.setAuthCompleteCallback)(async () => {
        ...
        mainWindow?.webContents.send("spotify:auth-complete", {
            refreshToken: Settings_1.Settings.credentials.refreshToken ? "present" : "missing"
        });
        // Get and send Spotify user info
        const userProfile = await SpotifyService_1.SpotifyService.getUserProfile();
        if (userProfile) {
    ...
    createTray();
    setupIPC();
    lyricsApp = new LyricsStatusApp_1.LyricsStatusApp();
    if (Settings_1.Settings.general.startMinimized) {
        mainWindow?.hide();
    }
    // Auto-start if credentials are configured
    if (Settings_1.Settings.credentials.token && Settings_1.Settings.credentials.refreshToken) {
        await lyricsApp.start();
        updateTrayMenu();
