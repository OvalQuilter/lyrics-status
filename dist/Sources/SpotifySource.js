...
class SpotifySource extends BaseSource_1.BaseSource {
    // Prefer the manually pasted web token over the OAuth token from SpotifyService —
    // the web token works for spclient endpoints. Falls back to SpotifyService.token if unset.
    getBearerToken() {
    ...
    getLyrics(name, artist) {
        return __awaiter(this, void 0, void 0, function* () {
            // Use the songId from the player state endpoint — SpotifyService already
            // called this to get playback state, but we need to call it ourselves here
            // because LyricsFetcher only passes name+artist, not the track ID.
            const playerRes = yield this.request("https://api.spotify.com/v1/me/player");
            ...
            const lyricsRes = yield this.request(`https://spclient.wg.spotify.com/color-lyrics/v2/track/${songId}?format=json&vocalRemoval=false&market=from_token`);
            if (!lyricsRes.ok) throw new Error(`Spotify lyrics HTTP ${lyricsRes.status}`);
            const json = yield lyricsRes.json();
            ...
            if (!json.lyrics.lines || json.lyrics.lines.length === 0)
                throw new Error("Spotify: empty lyrics lines");
