...
const LRC_RE_G = /\[(\d\d):(\d\d)(?:\.(\d\d))?]/g;
const LRC_RE   = /\[(\d\d):(\d\d)(?:\.(\d\d))?]/;
    ...
    parseLyrics(lyrics) {
        ...
        for (const line of lyrics.split("\n")) {
            ...
            while ((m = LRC_RE_G.exec(line)) !== null)
                times.push((+m[1] * 60 + +m[2]) * 1000 + (m[3] ? +m[3] * 10 : 0));
            const text = line.replace(LRC_RE, "").trim(); // Bug 6: use non-/g for replace
            if (!text) continue;
